...
// skill 根目录（向上两级从 scripts/ 到 wechat-article-writer/）
const __filename = fileURLToPath(import.meta.url);
const __skillDir = path.dirname(path.dirname(__filename));
...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const paths = [
    // 1. skill 自带扩展配置（便携核心）
    path.join(__skillDir, "EXTEND.md"),
    // 2. cwd 下的 baoyu-skills（工作区级配置）
    path.join(process.cwd(), ".baoyu-skills", "EXTEND.md"),
    // 3. XDG_CONFIG_HOME 路径
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"),
      "baoyu-skills", "EXTEND.md"
    ),
    // 4. 用户 home 路径（全局配置兜底）
    path.join(os.homedir(), ".baoyu-skills", "EXTEND.md"),
  ];
...
export function loadCredentials(account?: ResolvedAccount): { appId: string; appSecret: string } {
  ...
  // 优先级：1. skill 自带配置（便携核心）> 2. cwd > 3. home
  const skillEnvPath = path.join(__skillDir, "config", ".env");
  ...
  if (!appId) {
    appId = process.env.WECHAT_APP_ID || skillEnv.WECHAT_APP_ID || cwdEnv.WECHAT_APP_ID || homeEnv.WECHAT_APP_ID || "";
  }
  if (!appSecret) {
    appSecret = process.env.WECHAT_APP_SECRET || skillEnv.WECHAT_APP_SECRET || cwdEnv.WECHAT_APP_SECRET || homeEnv.WECHAT_APP_SECRET || "";
  }
  ...
  if (!appId || !appSecret) {
    ...
    throw new Error(
      ...
      "Set via EXTEND.md account config, environment variables, skill config/.env, or ~/.baoyu-skills/.env file."
    );
  }
